...
//     { name: 'Keyboard', price: 100, inStock: false }
//   ]
//   Uppercased names:  [ 'LAPTOP', 'PHONE', 'TABLET', 'MONITOR', 'KEYBOARD' ]
...
const discountProducts = products.map((product) => ({
    ...product, 
    price: discount20(product.price)
}));
...
//     { name: 'Tablet', price: 640, inStock: true },
...
*/
const addedStock = products.filter((product) => product.inStock).reduce((total, product) => total + product.price, 0);
console.log("Total value in stock: ", addedStock)
